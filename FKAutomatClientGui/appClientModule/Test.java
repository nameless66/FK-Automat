...
public class Test {
	...
	public static void main(String args[]) {
		...
		// System.out.println("Test.main:: prop" + i + " = " +
		// System.getProperty("prop" + i));
		// for (int i = 0; i < args.length; i++)
		...
		if (args.length < 1)
			...
			try {
				...
				KundeManagerInterface manager = (KundeManagerInterface) ctx
						.lookup("FKAutomatBeans/KundeManager!session.KundeManagerInterface");
				Kunde kunde = new Kunde();
				kunde.setNachname("Hallo");
				kunde.setVorname("Timo");

				System.out.println(kunde.toString());
				manager.insert(kunde);
				manager.insert(kunde);
				manager.insert(kunde);
				manager.insert(kunde);
				System.out.println("ssss");

				for (Kunde k : (ArrayList<Kunde>) manager.list())
					System.out.println("Test.main:: k = " + k);
